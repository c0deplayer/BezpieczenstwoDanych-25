...
class DeterministicAnonymizer:
    ...
    def _generate_numeric_noise(self, values: np.ndarray, field_name: str, 
    ...
    def _text_transform(self, text: str, field_name: str, reverse: bool = False) -> str:
        """
        ...
        for i, char in enumerate(text):
            ...
            h = hashlib.sha256(f"{seed}_{i}".encode('utf-8'))
            ...
            if not reverse:
                ...
                if CHAR_START <= char_code <= 126:
                    ...
                    new_char_code = ((char_code - CHAR_START + offset) % CHAR_RANGE) + CHAR_START
                else:
                ...
                if CHAR_START <= char_code <= 126:
                    ...
                    new_char_code = ((char_code - CHAR_START - offset + CHAR_RANGE) % CHAR_RANGE) + CHAR_START
                else:
    ...
    def anonymize(self, df: pd.DataFrame, config: Dict[str, Dict[str, Any]]) -> pd.DataFrame:
        """
    ...
    def deanonymize(self, df: pd.DataFrame, config: Dict[str, Dict[str, Any]]) -> pd.DataFrame:
